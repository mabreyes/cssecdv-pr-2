    ...
    body('username')
        ...
        .custom(async (value) => {
            ...
            return new Promise((resolve, reject) => {
                db.get('SELECT 1 FROM username_blacklist WHERE username = ?', [value.toLowerCase()], (err, row) => {
                    if (err) reject(err);
        ...
        .custom(async (value) => {
            ...
            return new Promise((resolve, reject) => {
                db.get('SELECT 1 FROM users WHERE LOWER(username) = ?', [value.toLowerCase()], (err, row) => {
                    if (err) reject(err);
...
const emailValidation = [
    body('email')
        ...
        .custom(async (value) => {
            ...
            return new Promise((resolve, reject) => {
                db.get('SELECT 1 FROM users WHERE LOWER(email) = ?', [value.toLowerCase()], (err, row) => {
                    if (err) reject(err);
                    if (row) reject(new Error('An account with this email already exists'));
                    resolve(true);
                });
            });
        })
